 ...
 *******************************************************************************/
package com.example.xiayong.framework_selflearn.opensourcelib.pulltorefresh;
...
import com.example.xiayong.framework_selflearn.opensourcelib.pulltorefresh.internal.FlipLoadingLayout;
import com.example.xiayong.framework_selflearn.opensourcelib.pulltorefresh.internal.LoadingLayout;
import com.example.xiayong.framework_selflearn.opensourcelib.pulltorefresh.internal.RotateLoadingLayout;
import com.example.xiayong.framework_selflearn.opensourcelib.pulltorefresh.internal.Utils;
import com.example.xiayong.framework_selflearn.opensourcelib.pulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		switch (action) {
			...
			// 设置变量，同时不拦截点击事件，这样listview获取到点击事件后，能有机会获取到移动事件等
	...
	private void pullEvent() {
		...
		switch (mCurrentMode) {
			...
			case PULL_FROM_START:
			default:
				// newScrollValue只能为负值，调用的scrollTo
				newScrollValue = Math.round(Math.min(initialMotionValue - lastMotionValue, 0) / FRICTION);
				itemDimension = getHeaderSize();
				break;
		}

		setHeaderScroll(newScrollValue); // 具体的滚动函数
